...
# PREENCHA ESTA INFORMAÇÃO PARA CONECTAR AO SEU BANCO DE DADOS
ORACLE_DSN = "host:porta/service_name"  # Ex: "detran" ou "192.168.1.50:1521/ORCL"

# As variáveis ORACLE_USER e ORACLE_PASSWORD foram removidas daqui,
# pois agora serão obtidas da tela de login.
...
class TaskManager:
    ...
    def fetch_and_reserve_batch(self, batch_size: int):
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                sql_fetch_pending = f"""
                    SELECT NU_PID, CO_DEDO, FILENAME, BOX_X, BOX_Y, BOX_W, BOX_H
                    FROM {TABLE_RECORTE}
                    WHERE TP_RECORTE_STATUS = :status_pendente
                """
    ...
    def process_image_batch(self, hands_to_process, progress_callback):
        ...
        if not os.path.isdir(IMAGE_FOLDER_PATH):
            messagebox.showerror("Erro de Configuração", f"O caminho das imagens não foi encontrado:\n{IMAGE_FOLDER_PATH}")
            return {}
        ...
        for i, hand in enumerate(hands_to_process):
            ...
            try:
                ...
                messagebox.showerror("Erro ao Carregar Imagem", f"Falha ao carregar {image_name}: {e}")
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                if not evaluated_tasks.empty:
                    ...
                    eval_data_insert = [
                        {'pid': int(index[0]), 'cid': int(index[1]),
                         ...
                         'op_id': OPERATOR_ID, 'ts': timestamp}
                    ...
                    eval_data_update = [
                        {'status': STATUS_CONCLUIDO, 'pid': int(index[0]), 'cid': int(index[1])}
                        for index, row in evaluated_tasks.iterrows()
                    ]
                ...
                if not unevaluated_tasks.empty:
                    ...
                    uneval_data = [
                        {'status': STATUS_PENDENTE, 'pid': int(index[0]), 'cid': int(index[1])}
...
class LoginScreen(ctk.CTkFrame):
    ...
    def login(self):
        ...
        if not username or not password:
            messagebox.showerror("Erro de Login", "Usuário e senha não podem estar em branco.")
            return

        try:
            dll_path = os.path.join("bin", "Senha.dll")
            if not os.path.exists(dll_path):
                messagebox.showerror("Erro Crítico", f"Não foi possível encontrar Senha.dll no diretório 'bin'.")
                return
            ...
            messagebox.showerror("Erro de DLL", f"Não foi possível encontrar o arquivo Senha.dll no diretório 'bin'.")
        except Exception as e:
            messagebox.showerror("Erro de Criptografia", f"Ocorreu um erro ao chamar a função da DLL: {e}")
...
class App(ctk.CTk):
    ...
    def attempt_login(self, username, password):
        try:
            ...
            # Erro já é exibido pelo construtor do TaskManager, apenas não prossiga.
            pass
...
class EvaluationScreen(ctk.CTkFrame):
    ...
    def on_closing(self, final_save=False):
        if not final_save:
            if not messagebox.askyesno("Confirmar Saída", "Tem certeza que deseja sair? Apenas as MÃOS 100% concluídas serão salvas."):
                return
